...
function buildItem(entry, index) {
  ...
  return `<div class="item" data-index="${index}" data-bit="1" data-date="${ts}" data-uploaded="${ts}" data-release="${ts}" data-assigned="1" data-yymmdd="${yymmdd}" data-size="0" data-ft='[]' data-status="Completed" data-genres='${genresJson}' data-rating="${rating}" data-w="680" data-h="1000"><a href="${cover}" target="_blank" rel="noopener"><div class="thumb loading"><img src="${cover}" loading="lazy"></div></a><div class="filename">${title}</div> <div class="dates"> <div class="date" data-longdate="${yymmdd}000000"> Status: Completed </div> <div class="date">Rating: ${rating}</div> </div></div>`;
}

...
function mapEntry(raw, data) {
  ...
  const dateAdded = validDate ? raw.myStartDate : new Date().toISOString().slice(0, 10);
  const published = data.published && data.published.from ? data.published.from.slice(0, 10) : null;
  const entry = {
    ...
    dateAdded,
    releaseDate: published || dateAdded
  };

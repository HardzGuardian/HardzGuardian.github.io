// Revert fix-timeline-position-label.js: restore the static newest/oldest
// .timeline-year labels (decluttered version from fix-timeline-edge-years.js)
// and remove the scroll-driven current-position date label.
...
for (const page of PAGES) {
  ...
  if (html.includes(htmlNew)) { html = html.replace(htmlNew, htmlOld); changed++; }
  if (html.includes(jsNew1)) { html = html.replace(jsNew1, jsOld1); changed++; }
  if (html.includes(jsNew2)) { html = html.replace(jsNew2, jsOld2); changed++; }
